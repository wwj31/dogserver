...
const (
	...
	DecideMasterShowDuration = 2 * time.Second  // 定庄广播后的动画播放时间
	DealShowDuration         = 1 * time.Second  // 发牌广播后的动画播放时间
...
func (m *Mahjong) CanSetGold(p *inner.PlayerInfo) bool {
	if m.fsm.State() == Ready || m.fsm.State() == Settlement {
		return true
	}
	return false
}
