...
func (s *StatePlaying) operateGang(p *mahjongPlayer, seatIndex int, card Card, ntf *outer.MahjongBTEOperaNtf) (err outer.ERROR) {
	...
	// 杠成功后算分
	gangSuccess := func(gangType checkCardType, opNtf *outer.MahjongBTEOperaNtf, loseScores map[int32]int64) {
		var (
		...
		if gangType == GangType4 {
			p.finalStatsMsg.TotalDarkGang++
		} else {
			p.finalStatsMsg.TotalLightGang++
		}
		...
		gangPeerIndex := len(s.peerRecords) // 杠牌位置，这个时候还没有记录上,所以取len的值
...
func (s *StatePlaying) allSeatsWithoutHu(ignoreSeat ...int) (result []int) {
	...
	for seatIndex := 0; seatIndex < int(s.gameParams().GamePlayerNumber); seatIndex++ {
