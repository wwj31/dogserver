...
func (s *StatePlaying) Enter() {
	s.Log().Infow("[NiuNiu] enter state playing", "room", s.room.RoomId, "params", *s.gameParams())
	s.actionTimerId = ""
...
func (s *StatePlaying) Leave() {
	s.cancelActionTimer()
	s.Log().Infow("[NiuNiu] leave state playing", "room", s.room.RoomId)
}
...
// 过牌
func (s *StatePlaying) pass(player *niuniuPlayer) {
...
// 打牌
func (s *StatePlaying) play(player *niuniuPlayer, cards PokerCards) outer.ERROR {
	if len(cards) == 0 {
...
func (s *StatePlaying) getPlayerAndSeatE(shortId int64) (*niuniuPlayer, int, outer.ERROR) {
